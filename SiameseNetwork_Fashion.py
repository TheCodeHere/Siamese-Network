...
batch_size = 128 # 256(Fashion, MNIST)
...
class DeviceDataLoader():
    ...
    def __iter__(self):
        ...
        for data in self.data:yield to_device(data, self.device)
...
# Model (on Device)
siameseNetwork_model = to_device(ResNet_size(size=18, in_channels=1, output_size=output_dim, margin=margin), device)

# Train ResNet
print(f"[{datetime.now()}]")
start = time.time()

fit(epochs=epochs, max_lr=max_lr, model=siameseNetwork_model, train_loader=train_ld, val_loader=val_ld, weight_decay=weight_decay, grad_clip=grad_clip, opt_func=opt_func)
...
File_name = "Log/SiameseNet/Fashion/" + str(output_dim) + "d/Fashion_acc-" + str(acc) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_m-" + str(margin) + ".pth"
