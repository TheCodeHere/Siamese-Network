...
batch_size = 256 # 256(Fashion, MNIST)
...
print("Train dataset size: ", len(siamese_train_ds))
print("Validation dataset size: ", len(siamese_val_ds), "\n")

#Dataset to Batches
siamese_train_ld = DataLoader(siamese_train_ds, batch_size=batch_size, shuffle=True)
siamese_val_ld = DataLoader(siamese_val_ds, batch_size=val_size, shuffle=True)
...
#Batches to GPU
train_ld = DeviceDataLoader(siamese_train_ld, device)
val_ld = DeviceDataLoader(siamese_val_ld, device)
...
class Siamese_Network(nn.Module):
    def __init__(self, block, num_blocks, in_channels=3, output_size=2, m=1.0, pow=2.0):
        ...
        self.dist = lambda a, b: nn.PairwiseDistance(p=pow, eps=0, keepdim=True)(a, b)
        self.contrastive_loss = lambda a, b, y: torch.mean((1-y) * torch.pow(self.dist(a,b), 2) + (y) * torch.pow(F.relu(m - self.dist(a,b)), 2))
        self.contrastive_loss_without_reduction = lambda a, b, y: (1-y) * torch.pow(self.dist(a,b), 2) + (y) * torch.pow(F.relu(m - self.dist(a,b)), 2)
    ...
    def training_step(self, batch):
        ...
        # Calculate loss
        loss = self.contrastive_loss(a, b, labels)
    ...
    def evaluate_step(self, val_loader):
        with torch.no_grad():
            ...
            for batch in val_loader:
                ...
                losses = self.contrastive_loss_without_reduction(a, b, labels)
...
def ResNet_size(size=10, in_channels=3, output_size=2, margin = 1.0):
    if size == 10:
        return Siamese_Network(BasicBlock, [1, 1, 1, 1], in_channels=in_channels, output_size=output_size, m=margin)
    elif size == 18:
        return Siamese_Network(BasicBlock, [2, 2, 2, 2], in_channels=in_channels, output_size=output_size, m=margin)
    elif size == 34:
        return Siamese_Network(BasicBlock, [3, 4, 6, 3], in_channels=in_channels, output_size=output_size, m=margin)
    elif size == 50:
        return Siamese_Network(Bottleneck, [3, 4, 6, 3], in_channels=in_channels, output_size=output_size, m=margin)
    elif size == 101:
        return Siamese_Network(Bottleneck, [3, 4, 23, 3], in_channels=in_channels, output_size=output_size, m=margin)
    elif size == 152:
        return Siamese_Network(Bottleneck, [3, 8, 36, 3], in_channels=in_channels, output_size=output_size, m=margin)
...
# Model (on Device)
siameseNetwork_model = to_device(ResNet_size(size=18, in_channels=3, output_size=output_dim, margin=margin), device)

# Train ResNet
print(f"[{datetime.now()}]")
start = time.time()

mean_loss = fit(epochs=epochs, max_lr=max_lr, model=siameseNetwork_model, train_loader=train_ld, val_loader=val_ld, weight_decay=weight_decay, grad_clip=grad_clip, opt_func=opt_func)
...
embeddings_plot, labels_plot = siameseNetwork_model.extract_embedding(train_dataset)
...
embeddings_plot, labels_plot = siameseNetwork_model.extract_embedding(test_dataset)
...
#####################################################################################################################
print("\nSAVING Siamese Network MODEL\n")
#The .state_dict method returns an OrderedDict containing all the weights and bias matrices mapped to the right attributes of the model

File_name = "Log/SiameseNet/Cifar10/" + str(output_dim) + "d/Cifar10_acc-" + str(acc) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_m-" + str(margin) + ".pth"
torch.save(siameseNetwork_model.state_dict(), File_name)
#####################################################################################################################
